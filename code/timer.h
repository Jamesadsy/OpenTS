...
class BasicTimerClass {
	public:
		...
		/*
		 * Frame regulators keep their raw start reading because frame state is saved.
		 * Process-origin regulators save elapsed time and rebuild their start reading
		 * from the loading process's clock. -1 remains the stopped-timer sentinel.
		 */
		template<typename S>
...
template<class T>
inline int BasicTimerClass<T>::Value(void) const
{
...
class TTimerClass : public BasicTimerClass<T> {
		typedef BasicTimerClass<T> BASECLASS;

		using BASECLASS::Started;
		using BASECLASS::Timer;
	public:
		// Constructor allows assignment as if class was integral 'long' type.
...
/*
 * This timer counts down from the specified (or constructed) value down towards zero,
 * just like CDTimerClass, but does so at a specified rate.
 */
template<class T>
class ProgressTimerClass : protected CDTimerClass<T>
{
		typedef CDTimerClass<T> BASECLASS;
	...
	protected:
		/*
		 * This is the full span the timer was constructed with. It is the denominator that
		 * the Progress function divides the elapsed time by, and it never changes thereafter.
		 */
		int TotalTime;
};
...
template<class T>
inline ProgressTimerClass<T>::ProgressTimerClass(int set) :
	BASECLASS(set),
	TotalTime(set)
{
}
...
template<class T>
inline int ProgressTimerClass<T>::Time_Spent(void) const
{
...
template<class T>
inline double ProgressTimerClass<T>::Progress(void) const
{
	if (TotalTime == 0) {
		return(1.0);
	}
	return((double)Time_Spent() / (double)TotalTime);
}


template<class T>
inline bool ProgressTimerClass<T>::Has_Completed(void) const
{
	return(Progress() == 1.0);
}

template<class T>
inline bool ProgressTimerClass<T>::Has_Completed2(void) const
{
	return(Progress() >= 1.0);
}
