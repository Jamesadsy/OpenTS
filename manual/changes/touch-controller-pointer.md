...
An SDL3 gamepad can move the same free pointer with its left stick from the first frontend screen, hold the primary or secondary mouse button, send Escape from Start, hold Control or Alt, navigate with the D-pad, and pan the tactical camera with the right stick; Circle in Focus Mode invokes the active screen's declared Back action. The Game Controls dialog offers eight `ScrollRate` positions, and right-stick pan scales from 1.0 at 0 to 0.5 at 7; touch pan keeps its existing speed. R2 boosts only pointer speed, and unsupported Square, Triangle, and stick-click actions remain inert.
