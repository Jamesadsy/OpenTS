...
Saved games keep to a `Saved Games` folder of their own, beside the game or inside the [user directory](/using/game-data/) when one is named. Campaign and skirmish `.SAV` files for the selected product use its `Tiberian Sun` or `Firestorm` child folder. Manual saves, quick saves, autosaves, load listings, and deletion all resolve within that product folder. The folders are created when the game first asks for a saved game. Multiplayer `.NET` saves and random-map settings remain directly in `Saved Games`. Unlike the files the game reads, a saved game is never looked for through the general file search path.

Earlier `.SAV` files directly in `Saved Games` remain untouched and are not shown in either product's load list. Their listing fields do not establish the selected product reliably enough for automatic migration. A player who knows which product created a legacy save can copy it into that product's child folder; the original may be kept as a backup.
